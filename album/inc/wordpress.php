...
function album_enqueue() {
	...
	// Styles.
	wp_enqueue_style( 'album-style', get_stylesheet_uri(), null, '1.5' );
	...
	wp_enqueue_script( 'album-script-main', get_theme_file_uri( '/assets/scripts/global.js' ), array( 'jquery' ), '1.0', false );
...
function album_fonts() {
	...
	if ( 'off' !== esc_html_x( 'on', 'Inconsolata: on or off', 'album' ) ) {
...
function album_after_setup_theme() {
	...
	register_nav_menus(
		array(
			...
			'menu-2' => esc_html__( 'Header Bottom', 'album' ),
		)
	);
