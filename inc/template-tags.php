 ...
 * @return string
 */
function terminal_human_time_diff() {
	...
	if ( $post_time > $time_now - ( 60 * 60 * 24 * 60 ) ) {
		...
		$human_time = sprintf( esc_html__( '%s ago', 'terminal' ), human_time_diff( $post_time, current_time( 'timestamp' ) ) );
 ...
 * @return boolean
 */
function terminal_archive_image_url( $post_id = null, $thumbnail_size = 'terminal-archive', $attr = array() ) {
 ...
 * @return string
 */
function terminal_post_thumbnail_html( $html, $post_id, $thumbnail_id, $size = '', $attr = array() ) {
...
add_filter( 'post_thumbnail_html', 'terminal_post_thumbnail_html', 10, 5 );


/**
 * Prints HTML with meta information for the current post-date or time.
 *
 * The time is generated by {@see terminal_human_time_diff}.
 */
function terminal_post_time() {

	$time_string = sprintf(
		'<time class="entry-date published updated" datetime="%1$s">%2$s</time>',
		esc_attr( get_the_date( 'c' ) ),
		terminal_human_time_diff()
	);
...
function terminal_post_author() {
	...
	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'terminal' ),
...
function terminal_comments_link() {
	...
	if ( ! post_password_required() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) {
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'terminal' ),
					array(
 ...
 * 'read more'.
 */
function terminal_read_more_text() {
 ...
 */
function terminal_contributor( $user_id = null, $post_count = null ) {
...
?>
		...
		<p class="buttons">
			<a href="<?php echo esc_url( get_author_posts_url( $user_id ) ); ?>"><?php esc_html_e( 'All Posts &rarr;', 'terminal' ); ?></a>

			<?php echo terminal_author_website(); ?>
		</p>
 ...
 */
function terminal_the_main_category() {
 ...
 */
function terminal_project_terms() {
...
?>
	<div class="projects-terms">
		...
		<strong class="project-terms-intro">
			<?php esc_html_e( 'Categories', 'terminal' ); ?>
		</strong>

		<a class="<?php echo esc_attr( $class ); ?>" href="<?php echo esc_url( home_url( '/portfolio/' ) ); ?>" data-tag="terminal-show-all"><?php esc_html_e( 'All', 'terminal' ); ?></a>
 ...
 * @return string
 */
function terminal_svg( $name, $echo = true ) {
 ...
 */
function terminal_author_website() {

	if ( get_the_author_meta( 'url' ) ) {
		return '<a href="' . esc_url( get_the_author_meta( 'url' ) ) . '" rel="author external">' . esc_html__( 'Website &rarr;', 'terminal' ) . '</a>';
	}
