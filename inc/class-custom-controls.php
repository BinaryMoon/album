 ...
 */
class Album_Category_Dropdown_Custom_Control extends WP_Customize_Control {
...
?>
	<label>
		...
		<select <?php $this->link(); ?>>
			<option value="-2" <?php echo selected( $value, -2, false ); ?>><?php echo esc_html__( 'No Categories (Hide)', 'album' ); ?></option>
			<option value="-1" <?php echo selected( $value, -1, false ); ?>><?php echo esc_html__( 'All Categories', 'album' ); ?></option>
<?php
		$args = array();
 ...
 */
class Album_Dropdown_Custom_Control extends WP_Customize_Control {
 ...
 */
class Album_DragDrop_List_Control extends WP_Customize_Control {
	...
	public function render_content() {
		...
		echo '<select class="album-dragdrop-select">';
		echo '<option disabled selected>' . esc_html__( 'Select a category to display +', 'album' ) . '</option>';
		foreach ( get_categories() as $category ) {
			if ( ! in_array( (int) $category->term_id, $values, true ) ) {
...
?>
		<input type="hidden" id="<?php echo absint( $this->id ); ?>" class="album-hidden-categories" <?php $this->link(); ?> value="<?php echo esc_html( $this->value() ); ?>">
<?php
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'album-theme-customizer', get_template_directory_uri() . '/styles/css/customizer.css' );
		wp_enqueue_script( 'album-theme-customizer', get_template_directory_uri() . '/scripts/theme-customizer.js', array( 'jquery' ), '1.0', true );
